...
namespace BMSANT
{
	...
	static constexpr uint16_t PacketRequestInerval = 250;
	static constexpr uint8_t CellsNumber = 32U;
	static constexpr uint8_t TempsNumber = 6U;
	...
	struct __attribute__((packed)) packet_raw_reverse_t
	{
		...
		uint16_t pulses_num;				// WAT?
		uint16_t tire_length;				// WAT?
		balancer_status_t status_balancer;	// Флаг состояние балансировки, balancer_status_t
		discharge_mosfet_status_t status_dcharge_fet;	// Флаг состояние ключа разрядки, discharge_mosfet_status_t
		charge_mosfet_status_t status_charge_fet;		// Флаг состояние ключа зарядки, charge_mosfet_status_t
		int16_t  temperature[TempsNumber];	// Температура датчиков 1..6, 1.0 C
		uint32_t uptime;					// Кол-во времени с питанием, 1.0 s
		uint32_t capacity_cycle;			// , 0.001 Ah
		uint32_t capacity_rem;				// Оставшаяся ёмкость АКБ, 0.000001 Ah
		uint32_t capacity_phy;				// Фактическая ёмкость АКБ, 0.000001 Ah
		uint8_t  capacity_percent;			// Оставшаяся ёмкость АКБ, 1.0 %
		int32_t  total_current;				// Ток разряда иди заряда, 0.1 A
		uint16_t cell_voltage[CellsNumber];	// Напряжение на ячейках 1..32, 0.001 V
		uint16_t total_voltage;				// Напряжение на всей АКБ, 0.1 V
		uint32_t header;					// Заголовок пакета.
	};
